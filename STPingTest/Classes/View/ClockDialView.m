 ...
 */
-(void)drawArcWithStartAngle:(CGFloat)startAngle endAngle:(CGFloat)endAngle lineWidth:(CGFloat)lineWitdth fillColor:(UIColor*)filleColor strokeColor:(UIColor*)strokeColor isSpeed:(BOOL)isSpeed{
    //保存弧线宽度,开始角度，结束角度
    ...
    UIBezierPath *bigArc=[UIBezierPath bezierPathWithArcCenter:LuCenter radius:self.arcRadius +5 startAngle:startAngle endAngle:endAngle clockwise:YES];
    CAShapeLayer* bigShapeLayer=[CAShapeLayer layer];
    bigShapeLayer.lineWidth=5;
    bigShapeLayer.fillColor=filleColor.CGColor;
    if (isSpeed) {
        bigShapeLayer.strokeColor=[UIColor colorWithHex:0x6A11CB alpha:0.5].CGColor;
        self.innerCursorImageView.image = [UIImage imageNamed:@"指针"];
//        self.innerCursorImageView.frame = CGRectMake(self.bounds.size.width / 2 - 10, Calculate_radius - 50, self.innerCursorImageView.image.size.width * 0.5, self.innerCursorImageView.image.size.height * 0.5);
    }else
    {
        bigShapeLayer.strokeColor=RGBA(247,107,28,0.70).CGColor;
        self.innerCursorImageView.image = [UIImage imageNamed:@"指针1"];
    }
    bigShapeLayer.path=bigArc.CGPath;
    bigShapeLayer.lineCap=kCALineCapRound;
    ...
    UIBezierPath *midlleArc=[UIBezierPath bezierPathWithArcCenter:LuCenter radius:self.arcRadius - 2.5 startAngle:startAngle endAngle:endAngle clockwise:YES];
    CAShapeLayer* midlleShapeLayer=[CAShapeLayer layer];
    midlleShapeLayer.lineWidth=lineWitdth;
    midlleShapeLayer.fillColor=filleColor.CGColor;
    if (isSpeed) {
        midlleShapeLayer.strokeColor=[UIColor colorWithHex:0x6A11CB alpha:0.5].CGColor;
    }else
    {
        midlleShapeLayer.strokeColor=RGBA(247,107,28,0.70).CGColor;
    }
    midlleShapeLayer.path=midlleArc.CGPath;
    midlleShapeLayer.lineCap=kCALineCapRound;
    [self.layer addSublayer:midlleShapeLayer];
...
- (UIImageView *)innerCursorImageView {
    if (!_innerCursorImageView) {
        ...
        innerCursorImageView.image = [UIImage imageNamed:@"指针1"];
        innerCursorImageView.frame = CGRectMake(self.bounds.size.width / 2 - 13, Calculate_radius - 50, innerCursorImageView.image.size.width * 0.5, innerCursorImageView.image.size.height * 0.5);
        // 旋转成和仪表盘角度一致
